...
for subject in subjects:
    ...
    for epoch_params in epochs_params:
        ...
        if 'resample' in decoding_preproc.keys():
            epochs = my_resample(epochs, decoding_preproc['resample'])
        if 'decim' in decoding_preproc.keys():
            epochs = my_decim(epochs, decoding_preproc['decim'])
        if 'crop' in decoding_preproc.keys():
            epochs.crop(decoding_preproc['crop']['tmin'],
                        decoding_preproc['crop']['tmax'])
